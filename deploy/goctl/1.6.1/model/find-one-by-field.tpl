...
{{if .withCache}}{{.cacheKey}}
	...
	if err := conn.QueryRowCtx(ctx, &resp, query, {{.lowerStartCamelField}},globalkey.Sql_DelStateNo); err != nil {
		return nil, err
	}
	...
	query := fmt.Sprintf("select %s from %s where {{.originalField}}  and del_state = ? limit 1", {{.lowerStartCamelObject}}Rows, m.table )
	err := m.conn.QueryRowCtx(ctx, &resp, query, {{.lowerStartCamelField}},globalkey.Sql_DelStateNo)
	switch err {
	case nil:
		return &resp, nil
	case sqlc.ErrNotFound:
